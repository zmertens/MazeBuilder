...
void world::create_world(int p, int q, world_func func, Map* m, int chunk_size) noexcept {
    ...
    double world::get_time() const noexcept {
        return (static_cast<double>(SDL_GetTicks()) + static_cast<double>(m_model.start_time) - static_cast<double>(m_model.start_ticks)) / 1000.0;
    }
    ...
    std::uint32_t world::gen_plant_buffer(const float x, const float y, const float z, const float n, const int w) const noexcept {
    ...
    std::uint32_t world::gen_player_buffer(const float x, const float y, const float z, const float rx, const float ry) const noexcept {
        GLfloat* data = malloc_faces(10, 6);
    ...
    void world::draw_lines(const Attrib* attrib, const std::uint32_t buffer, const int components, const int count) const noexcept {
        glBindBuffer(GL_ARRAY_BUFFER, buffer);
        glEnableVertexAttribArray(attrib->position);
...
const world::Player* world::find_player(const int id) const noexcept {
    ...
    int world::hit_test(const int previous, const float x, const float y,
        ...
        for (int i = 0; i < m_model.chunk_count; i++) {
            ...
            if (hw > 0) {
                if (float d = SDL_sqrtf(SDL_powf(hx - x, 2) + SDL_powf(hy - y, 2) + SDL_powf(hz - z, 2)); best == 0 || d < best) {
                    best = d;
                    *bx = hx; *by = hy; *bz = hz;
                    result = hw;
                }
    ...
    void world::occlusion(char neighbors[27], char lights[27], float shades[27], float ao[6][4], float light[6][4]) const noexcept {

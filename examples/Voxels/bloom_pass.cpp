...
bool render_pass::init(const int w, const int h,
                       ...
                       const bool depth) noexcept
{
    ...
    {
        glGenTextures(1, &color_tex[static_cast<std::size_t>(i)]);
        glBindTexture(GL_TEXTURE_2D, color_tex[static_cast<std::size_t>(i)]);
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER,
                               ...
                               GL_TEXTURE_2D,
                               color_tex[static_cast<std::size_t>(i)], 0);
...
void render_pass::bind() const noexcept
{
    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
}

void render_pass::unbind() noexcept
{
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
}

void render_pass::destroy() noexcept
{
    for (int i = 0; i < num_color_attachments; ++i)
    {
        if (color_tex[static_cast<std::size_t>(i)])
        {
            glDeleteTextures(1, &color_tex[static_cast<std::size_t>(i)]);
            color_tex[static_cast<std::size_t>(i)] = 0;
        }
    }
                      ...
                      const std::uint32_t composite_program) noexcept
{
    ...
    return m_scene_pass.fbo != 0 && m_blur_pass[0].fbo != 0 && m_blur_pass[1].fbo != 0;
}
...
void bloom_pass::execute(const float strength) const noexcept
{
    ...
    GLuint src_tex = m_scene_pass.color_tex[1];
    ...
    if (strength > 0.0f)
    {
        ...
        for (int i = 0; i < BLUR_PASSES * 2; ++i)
        {
            ...
            src_tex    = m_blur_pass[dst_idx].color_tex[0];
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, m_scene_pass.color_tex[0]);  // full scene
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, src_tex);                    // blurred bright pass

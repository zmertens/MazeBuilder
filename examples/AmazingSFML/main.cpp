...
namespace
{
    ...
    constexpr unsigned int MAZE_ROWS = WINDOW_WIDTH / MAZE_CELL_SIZE;
    constexpr unsigned int MAZE_COLS = WINDOW_HEIGHT / MAZE_CELL_SIZE;
    ...
    {
        enum class WallColor : std::uint16_t
        {
            R1 = 40,
            G1 = 40,
            B1 = 60,
        ...
        static constexpr std::tuple<std::uint16_t, std::uint16_t, std::uint16_t> WALL_COLOR_VALUES[]{
            {static_cast<std::uint16_t>(WallColor::R1), static_cast<std::uint16_t>(WallColor::G1),
             static_cast<std::uint16_t>(WallColor::B1)},
            {static_cast<std::uint16_t>(WallColor::R2), static_cast<std::uint16_t>(WallColor::G2),
             static_cast<std::uint16_t>(WallColor::B2)},
            {static_cast<std::uint16_t>(WallColor::R3), static_cast<std::uint16_t>(WallColor::G3),
             static_cast<std::uint16_t>(WallColor::B3)}};
    ...
    public:
        ...
        {
            const auto idx = static_cast<std::size_t>(RNG.get_int(0, static_cast<int>(std::size(WALL_COLOR_VALUES) - 1)));
            const auto &[r, g, b] = WALL_COLOR_VALUES[idx];
            return sf::Color{
        ...
        private:
            ...
            void create_world_boundaries()
            {
                ...
                const float world_w = static_cast<float>(m_maze->columns) * (MAZE_CELL_SIZE + MAZE_WALL_SIZE);
                const float world_h = static_cast<float>(m_maze->rows) * (MAZE_CELL_SIZE + MAZE_WALL_SIZE);
            ...
            void rebuild_maze()
            {
                ...
                request = "--rows=" + std::to_string(MAZE_ROWS) +
                          ...
                          " --output=" + MAZE_TEMP_IMAGE_PATH.string() + " --distances";
                ...
                const float expected_width = static_cast<float>(MAZE_COLS) * (MAZE_CELL_SIZE + MAZE_WALL_SIZE) + MAZE_WALL_SIZE;
                const float expected_height = static_cast<float>(MAZE_ROWS) * (MAZE_CELL_SIZE + MAZE_WALL_SIZE) + MAZE_WALL_SIZE;
                ...
                m_maze = build_topology_from_grid(*generated_grid);

                build_geometry_and_physics();

                for (int i = 0; i < 24; ++i)
                {
                    const float x = (MAZE_CELL_SIZE + MAZE_WALL_SIZE) * (1.0f + static_cast<float>(RNG(0, static_cast<int>(m_maze->columns - 2u))));

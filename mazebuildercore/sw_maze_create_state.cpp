...
sw_maze_create_state::sw_maze_create_state(const runtime_app::context &ctx, runtime_stack *stack)
: state(ctx, stack), grid_mapper{ctx.get_grid_manager()}, processed_text_mapper{ctx.get_text_manager()}
{

}

std::string_view sw_maze_create_state::create(algo a, unsigned int rows, unsigned int cols, unsigned int levels, randomizer &rng) noexcept
{
    if (a != algo::SIDEWINDER)
    {
        return std::string_view{"Unsupported algorithm"};
    }

    return create_sw_maze(rows, cols, levels, std::ref(rng));   
}
...
bool sw_maze_create_state::update([[maybe_unused]] const std::optional<args> &args, [[maybe_unused]] double delta_time) noexcept
{
    ...
    auto result = create(algo::SIDEWINDER, rows, cols, levels, *rng_ptr);
    if (!result.empty())
    {
        try
        {
...
std::string_view sw_maze_create_state::create_sw_maze(unsigned int rows, unsigned int cols, unsigned int levels, randomizer &rng) noexcept
{
        ...
        for (unsigned int lv = 0; lv < levels; ++lv)
        {
            for (unsigned int row = 0; row < rows; ++row)
            {
                ...
                for (unsigned int col = 0; col < cols; ++col)
                {
                    ...
                    const bool close    = at_east || (!at_north && rng(0, 1) == 0);
                    ...
                    if (close)
                    {
                        if (!at_north && !run.empty())
                        {
                            ...
                            if (auto n = grid_ops.get_north(m)) lab::link(m, n, true);
                        }
                        run.clear();
                    }
                    else
                    {
                        if (auto e = grid_ops.get_east(c)) lab::link(c, e, true);
                    }
                }
            }
        }

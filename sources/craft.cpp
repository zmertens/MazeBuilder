/*
Craft engine originally written in C99
Ported to C++11
*/
...
float craft::get_daylight() const {
...
int craft::get_scale_factor() const {
...
void craft::get_sight_vector(float rx, float ry, float *vx, float *vy, float *vz) const {
    float m = SDL_cosf(ry);
...
void craft::get_motion_vector(int flying, int sz, int sx, float rx, float ry,
...
GLuint craft::gen_crosshair_buffer() {
...
GLuint craft::gen_wireframe_buffer(float x, float y, float z, float n) {
    float data[72];
    make_cube_wireframe(data, x, y, z, n);
    return gen_buffer(sizeof(data), data);
}

GLuint craft::gen_sky_buffer() {
    float data[12288];
    make_sphere(data, 1, 3);
    return gen_buffer(sizeof(data), data);
}

GLuint craft::gen_cube_buffer(float x, float y, float z, float n, int w) {
...
GLuint craft::gen_plant_buffer(float x, float y, float z, float n, int w) {
...
GLuint craft::gen_player_buffer(float x, float y, float z, float rx, float ry) {
    GLfloat *data = malloc_faces(10, 6);
    make_player(data, x, y, z, rx, ry);
    return gen_faces(10, 6, data);
}

GLuint craft::gen_text_buffer(float x, float y, float n, char *text) {
...
void craft::draw_triangles_3d_ao(Attrib *attrib, GLuint buffer, int count) {
...
void craft::draw_triangles_3d_text(Attrib *attrib, GLuint buffer, int count) {
    glBindBuffer(GL_ARRAY_BUFFER, buffer);
    glEnableVertexAttribArray(attrib->position);
    glEnableVertexAttribArray(attrib->uv);
    glVertexAttribPointer(attrib->position, 3, GL_FLOAT, GL_FALSE,
...
void craft::draw_triangles_2d(Attrib *attrib, GLuint buffer, int count) {
    glBindBuffer(GL_ARRAY_BUFFER, buffer);
    glEnableVertexAttribArray(attrib->position);
    glEnableVertexAttribArray(attrib->uv);
...
void craft::draw_lines(Attrib *attrib, GLuint buffer, int components, int count) {
    glBindBuffer(GL_ARRAY_BUFFER, buffer);
    glEnableVertexAttribArray(attrib->position);
...
void craft::draw_chunk(Attrib *attrib, Chunk *chunk) {
    draw_triangles_3d_ao(attrib, chunk->buffer, chunk->faces * 6);
}

void craft::draw_item(Attrib *attrib, GLuint buffer, int count) {
    draw_triangles_3d_ao(attrib, buffer, count);
}

void craft::draw_text(Attrib *attrib, GLuint buffer, int length) {
    glEnable(GL_BLEND);
...
void craft::draw_signs(Attrib *attrib, Chunk *chunk) {
...
void craft::draw_sign(Attrib *attrib, GLuint buffer, int length) {
    glEnable(GL_POLYGON_OFFSET_FILL);
    glPolygonOffset(-8, -1024);
...
void craft::draw_cube(Attrib *attrib, GLuint buffer) {
    draw_item(attrib, buffer, 36);
}

void craft::draw_plant(Attrib *attrib, GLuint buffer) {
    draw_item(attrib, buffer, 24);
}

void craft::draw_player(Attrib *attrib, Player *player) {
    draw_cube(attrib, player->buffer);
}

craft::Player *craft::find_player(int id) {
...
void craft::update_player(Player *player, float x, float y, float z, float rx, float ry, int interpolate)
{
    if (interpolate) {
...
void craft::interpolate_player(Player *player) {
    State *s1 = &player->state1;
    State *s2 = &player->state2;
    float t1 = s2->t - s1->t;
...
void craft::delete_player(int id) {
...
float craft::player_player_distance(Player *p1, Player *p2) {
    State *s1 = &p1->state;
    State *s2 = &p2->state;
    float x = s2->x - s1->x;
    float y = s2->y - s1->y;
...
float craft::player_crosshair_distance(Player *p1, Player *p2) {
    State *s1 = &p1->state;
    State *s2 = &p2->state;
...
craft::Player *craft::player_crosshair(Player *player) {
    Player *result = 0;
...
craft::Chunk *craft::find_chunk(int p, int q) {
...
int craft::chunk_distance(Chunk *chunk, int p, int q) {
    int dp = ABS(chunk->p - p);
    int dq = ABS(chunk->q - q);
    return MAX(dp, dq);
}

int craft::chunk_visible(float planes[6][4], int p, int q, int miny, int maxy) {
...
int craft::highest_block(float x, float z) {
    int result = -1;
    int nx = SDL_roundf(x);
    int nz = SDL_roundf(z);
    int p = chunked(x);
...
int craft::hit_test_face(Player *player, int *x, int *y, int *z, int *face) {
    State *s = &player->state;
...
int craft::collide(int height, float *x, float *y, float *z) {
    int result = 0;
    int p = chunked(*x);
    int q = chunked(*z);
    Chunk *chunk = find_chunk(p, q);
...
int craft::player_intersects_block(
    int height,
    float x, float y, float z,
    int hx, int hy, int hz)
{
...
void craft::gen_sign_buffer(Chunk *chunk) {
...
int craft::has_lights(Chunk *chunk) {
    if (!SHOW_LIGHTS) {
        return 0;
    }
...
void craft::dirty_chunk(Chunk *chunk) {
    chunk->dirty = 1;
    if (has_lights(chunk)) {
        for (int dp = -1; dp <= 1; dp++) {
            for (int dq = -1; dq <= 1; dq++) {
...
void craft::compute_chunk(WorkerItem *item) {
...
void craft::generate_chunk(Chunk *chunk, WorkerItem *item) {
...
void craft::gen_chunk_buffer(Chunk *chunk) {
    WorkerItem _item;
...
void craft::load_chunk(WorkerItem *item) {
    int p = item->p;
    int q = item->q;
...
void craft::create_chunk(Chunk *chunk, int p, int q) {
...
void craft::force_chunks(Player *player) {
    State *s = &player->state;
    int p = chunked(s->x);
    int q = chunked(s->z);
    int r = 1;
...
void craft::ensure_chunks_worker(Player *player, Worker *worker) {
    State *s = &player->state;
    float matrix[16];
    set_matrix_3d(
...
void craft::ensure_chunks(Player *player) {
    check_workers();
    force_chunks(player);
    for (int i = 0; i < WORKERS; i++) {
        Worker *worker = g->workers + i;
...
void craft::unset_sign(int x, int y, int z) {
    int p = chunked(x);
    int q = chunked(z);
    Chunk *chunk = find_chunk(p, q);
    if (chunk) {
...
void craft::unset_sign_face(int x, int y, int z, int face) {
    int p = chunked(x);
    int q = chunked(z);
    Chunk *chunk = find_chunk(p, q);
    if (chunk) {
...
void craft::_set_sign(int p, int q, int x, int y, int z, int face, const char *text, int dirty) {
    if (strlen(text) == 0) {
...
void craft::set_sign(int x, int y, int z, int face, const char *text) {
    int p = chunked(x);
    int q = chunked(z);
...
void craft::set_light(int p, int q, int x, int y, int z, int w) {
    Chunk *chunk = find_chunk(p, q);
    if (chunk) {
        Map *map = &chunk->lights;
...
void craft::_set_block(int p, int q, int x, int y, int z, int w, int dirty) {
    Chunk *chunk = find_chunk(p, q);
    if (chunk) {
        Map *map = &chunk->map;
        if (map_set(map, x, y, z, w)) {
...
void craft::set_block(int x, int y, int z, int w) {
    int p = chunked(x);
    int q = chunked(z);
...
int craft::get_block(int x, int y, int z) {
    int p = chunked(x);
    int q = chunked(z);
    Chunk *chunk = find_chunk(p, q);
    if (chunk) {
...
void craft::builder_block(int x, int y, int z, int w) {
    if (y <= 0 || y >= 256) {
        return;
    }
...
int craft::render_chunks(Attrib *attrib, Player *player) {
    int result = 0;
    State *s = &player->state;
...
void craft::render_signs(Attrib *attrib, Player *player) {
    State *s = &player->state;
    int p = chunked(s->x);
    int q = chunked(s->z);
    float matrix[16];
...
void craft::render_sign(Attrib *attrib, Player *player) {
...
void craft::render_players(Attrib *attrib, Player *player) {
    State *s = &player->state;
    float matrix[16];
    set_matrix_3d(
        matrix, g->width, g->height,
...
void craft::render_sky(Attrib *attrib, Player *player, GLuint buffer) {
    State *s = &player->state;
    float matrix[16];
    set_matrix_3d(
        matrix, g->width, g->height,
...
void craft::render_wireframe(Attrib *attrib, Player *player) {
    State *s = &player->state;
    float matrix[16];
    set_matrix_3d(
        matrix, g->width, g->height,
...
void craft::render_crosshairs(Attrib *attrib) {
    float matrix[16];
...
void craft::render_item(Attrib *attrib) {
    float matrix[16];
...
void craft::render_text(Attrib *attrib, int justify, float x, float y, float n, char *text) {
    float matrix[16];
    set_matrix_2d(matrix, g->width, g->height);
    glUseProgram(attrib->program);
    glUniformMatrix4fv(attrib->matrix, 1, GL_FALSE, matrix);
...
void craft::add_message(const char *text) {
    printf("%s\n", text);
    snprintf(
...
void craft::array(Block *b1, Block *b2, int xc, int yc, int zc) {
    if (b1->w != b2->w) {
        return;
    }
    int w = b1->w;
...
void craft::cube(Block *b1, Block *b2, int fill) {
    if (b1->w != b2->w) {
        return;
    }
    int w = b1->w;
...
void craft::sphere(Block *center, int radius, int fill, int fx, int fy, int fz) {
...
void craft::cylinder(Block *b1, Block *b2, int radius, int fill) {
    if (b1->w != b2->w) {
        return;
    }
    int w = b1->w;
...
int craft::handle_events(double dt) {
...
void craft::create_window_and_context() {
...
void craft::parse_buffer(char *buffer) {
    Player *me = g->players;
    State *s = &g->players->state;
    char *key;

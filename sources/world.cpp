...
void world::create_world(int p, int q, world_func func, Map* m, const int CHUNK_SIZE, const bool SHOW_TREES, const bool SHOW_PLANTS, const bool SHOW_CLOUDS,
    unsigned int height, const std::vector<std::tuple<std::string, int, int>>& maze_parts) const noexcept {
    ...
    for (int dx = -pad; dx < CHUNK_SIZE + pad; dx++) {
        for (int dz = -pad; dz < CHUNK_SIZE + pad; dz++) {
            ...
            int base_z = q * CHUNK_SIZE + dz;
            

            // count rows
            for (auto&& maze_part : maze_parts) {
                auto line = get<0>(maze_part);
                int start_from_this_row_in_maze = get<1>(maze_part);
                int offset = start_from_this_row_in_maze + get<2>(maze_part);
                int z = base_z + offset;
                ...
                for (char c : line) {
                    if (c == '+' || c == '-' || c == '|') {
                        ...
                        // Put the maze build at a starting height in craft

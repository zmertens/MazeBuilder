...
void main() {
    ...
    ao += fragment_light;//min(1.0, ao + fragment_light);
    df += fragment_light;//min(1.0, df + fragment_light);
    float value = daylight + fragment_light;//min(1.0, daylight + fragment_light);
    vec3 light_color = vec3(value * 0.3 + 0.2);
    vec3 ambient = vec3(value * 0.3 + 0.2);
    vec3 light = ambient + light_color * df;
    color *= light * ao; //clamp(color * light * ao, vec3(0.0), vec3(1.0));

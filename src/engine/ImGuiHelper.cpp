 ...
 * @param rm
 */
ImGuiHelper::ImGuiHelper(const SdlWindow& sdl, ResourceManager& rm)
: cSdl(sdl)
, mResourceManager(rm)
...
void ImGuiHelper::render()
{
    ...
    int windowWidth = cSdl.getWindowWidth();
    int windowHeight = cSdl.getWindowHeight();

    // set player stats
    if (mShowHealth)
    {
        ImGui::SetNextWindowPos(ImVec2(0.0f, 0.0f));
        ImGui::Begin("#Stats Window", &mShowHealth,
            ImVec2(0.05f * static_cast<float>(windowWidth), 0.05f * static_cast<float>(windowHeight)),
            mOverlayAlpha, mOverlayFlags);
        ImGui::Text(mHealth.c_str(), "%s");
        ImGui::SameLine();
        ImGui::End();
    }
 ...
 * @return
 */
Gui::States ImGuiHelper::getState() const
{
    return mState;
}
 ...
 * @param state
 */
void ImGuiHelper::setState(const Gui::States& state)
{
    mState = state;
}
...
void ImGuiHelper::ImGui_ImplSdlGL3_NewFrame()
{
    ...
    if (SDL_GetWindowFlags(cSdl.getSdlWindow()) & SDL_WINDOW_MOUSE_FOCUS)
